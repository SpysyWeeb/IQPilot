...
def turn_desire(turn_direction) -> log.Desire:
  return TURN_DESIRES[getattr(turn_direction, "raw", turn_direction)]
...
class DesireHelper:
  ...
  def update(self, carstate, lateral_active, lane_change_prob, nav_state=None, modeldata=None, radar_state=None):
    ...
    lane_change_requested = one_blinker or nav_exit_active
    ...
    if not lateral_active or self.lane_change_timer > LANE_CHANGE_TIME_MAX or \
       (self.alc.lane_change_set_timer == AutoLaneChangeMode.OFF and not nav_exit_active):
      self.lane_change_state = LaneChangeState.off
      self.lane_change_direction = LaneChangeDirection.none
    else:
      # LaneChangeState.off -- enter on the rising edge of a blinker OR a nav exit request
      if self.lane_change_state == LaneChangeState.off and not below_lane_change_speed and \
         ((one_blinker and not self.prev_one_blinker) or (nav_exit_active and not self.prev_nav_exit_active)):
        self.lane_change_state = LaneChangeState.preLaneChange
        self.lane_change_ll_prob = 1.0
        self.lane_change_direction = self.get_lane_change_direction(carstate) if one_blinker else self.nav_exit.direction
      ...
      # LaneChangeState.preLaneChange
        ...
        nav_exit_start = nav_exit_active and self.nav_exit.auto_allowed

        if not lane_change_requested or below_lane_change_speed:
          self.lane_change_state = LaneChangeState.off
          self.lane_change_direction = LaneChangeDirection.none
        elif (torque_applied or self.alc.auto_lane_change_allowed or nav_exit_start) and not blindspot_detected:
    ...
    if self.nav_turn_direction != TurnDirection.none:
      self.desire = turn_desire(self.nav_turn_direction)
    elif self.lane_turn_direction != TurnDirection.none:
      self.desire = turn_desire(self.lane_turn_direction)
    else:
