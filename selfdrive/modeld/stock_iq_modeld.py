...
def _compute_action(outputs: dict[str, np.ndarray], previous: log.ModelDataV2.Action,
                    lat_horizon: float, long_horizon: float, speed_mps: float) -> log.ModelDataV2.Action:
  plan = outputs["plan"][0]
...
class LegacyVisionPolicyRuntime(ModelStateBase):
...
def run_stock_modeld(demo: bool = False):
  ...
  while True:
    ...
    if sm.frame % 60 == 0:
      runtime.lat_delay = get_lat_delay(params, sm["liveDelay"].lateralDelay)
    lat_horizon = runtime.lat_delay + LAT_SMOOTH_SECONDS
