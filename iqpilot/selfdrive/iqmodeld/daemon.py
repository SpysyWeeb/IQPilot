...
from openpilot.iqpilot.models.runtime_state import ModelStateBase
...
try:
  from openpilot.iqpilot.selfdrive.iqmodeld.native.iqmodel_pyx import CLContext, DrivingModelFrame
...
class NeuralEngineState(ModelStateBase):
  ...
  def __init__(self, gpu_context: CLContext):
    ...
    self.frames = {
      stream_name: DrivingModelFrame(gpu_context, context_depth)
  ...
  def get_action_from_model(self, outputs: dict[str, np.ndarray], previous_action: log.ModelDataV2.Action,
                            lat_action_t: float, long_action_t: float, vehicle_speed: float) -> log.ModelDataV2.Action:
    if "action" in outputs:
      ...
      if vehicle_speed > self.MIN_LAT_CONTROL_SPEED:
        curvature_cmd = smooth_value(curvature_cmd, previous_action.desiredCurvature, self.LAT_SMOOTH_SECONDS)
      else:
...
class CalibrationAtlas:
  ...
  def refresh(self, sm: SubMaster, main_is_wide: bool, dual_camera: bool) -> tuple[np.ndarray, np.ndarray, bool]:
    if not (sm.updated["liveCalibration"] and sm.seen["roadCameraState"] and sm.seen["deviceState"]):
...
class InferenceDaemon:
  def __init__(self, demo: bool = False):
    ...
    self._pub = PubMaster(["modelV2", "drivingModelData", "cameraOdometry", "iqDriveModelData"])
    self._sub = SubMaster([
  ...
  def _refresh_tunables(self, tick: int) -> None:
    if tick % 60 != 0:
      return
    self._runtime.lat_delay = get_lat_delay(self._params, self._sub["liveDelay"].lateralDelay)
  ...
  def _publish(self, outputs: dict[str, np.ndarray], main_stamp: CaptureStamp, extra_stamp: CaptureStamp,
    ...
    action = self._runtime.get_action_from_model(outputs, self._previous_action, lat_horizon, long_horizon, vehicle_speed)
  ...
  def serve(self) -> None:
    ...
    while True:
      ...
      lat_horizon = self._runtime.lat_delay + self._runtime.LAT_SMOOTH_SECONDS + DT_MDL
      ...
      outputs = self._runtime.run(vision_bufs, warp_map, fresh_inputs, prepare_only)
