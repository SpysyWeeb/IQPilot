...
def _captured_queue_depth(jit) -> int | None:
...
def validate_supercombo_release(run_policy_jit, model_runner, model_metadata, frame_skip, expected_device: str) -> None:
  ...
  zero_inputs = {
    'img': Tensor(np.zeros(image_shape, dtype=np.uint8), device=Device.DEFAULT).realize(),
    'big_img': Tensor(np.zeros(image_shape, dtype=np.uint8), device=Device.DEFAULT).realize(),
  }
...
if __name__ == "__main__":
  ...
  for cam_w, cam_h in args.camera_resolutions:
    ...
    captured_depth = _captured_queue_depth(out[(cam_w,cam_h)])
    expected_depth = args.frame_skip * ((out['metadata']['input_shapes']['img'][1] // 6) - 1) + 1
    if captured_depth is not None and captured_depth != expected_depth:
      raise AssertionError(
        f"compiled warp queue-depth mismatch for {cam_w}x{cam_h}: captured {captured_depth} expected {expected_depth}"
      )
