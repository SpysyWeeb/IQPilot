...
def _legacy_indices(shape: tuple[int, ...], mode: str) -> np.ndarray | None:
  if mode == "split":
    full = 100
...
def test_replay_ledger_layout_matches_expected_history(monkeypatch: pytest.MonkeyPatch,
  ...
  for tensor_name, tensor_shape in shapes.items():
    ...
    expected_selector = _legacy_indices(tensor_shape, mode)
...
def test_replay_ledger_rising_edge_and_hidden_state_updates(monkeypatch: pytest.MonkeyPatch):
  ...
  pulse[3] = 1.0
  state.run({}, {}, {"desire": pulse}, prepare_only=False)
  ...
  state.run({}, {}, {"desire": np.zeros(8, dtype=np.float32)}, prepare_only=False)
