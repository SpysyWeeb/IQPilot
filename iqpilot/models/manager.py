...
from openpilot.iqpilot.models.helpers import bundle_files_ready, get_active_bundle, get_runtime_bundle_upgrade, persist_active_bundle
...
class IQModelManager(_BaseIQModelManager):
  def _queue_tinygrad_upgrade(self) -> None:
    ...
    if self._download_index() is None and getattr(replacement, "index", None) is not None:
      ...
      cloudlog.warning(f"Queued tinygrad upgrade for legacy bundle {getattr(self.active_bundle, 'internalName', '<unknown>')}")
