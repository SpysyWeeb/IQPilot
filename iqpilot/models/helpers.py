...
def _load_cached_manifest_bundles(params: Params):
  ...
  for raw_bundle in cached.get("bundles", []):
    try:
      ...
      if "is_bundle_version_compatible" in globals() and not is_bundle_version_compatible(compatibility_view):
        continue

      if "short_name" in raw_bundle:
        from openpilot.iqpilot.models.fetcher import ModelParser
        bundles.append(ModelParser._parse_bundle(raw_bundle))
        continue
...
def get_active_bundle(params: Params = None):
  ...
  try:
    ...
    if not active_bundle:
      return None
    if "is_bundle_version_compatible" in globals() and not is_bundle_version_compatible(active_bundle):
      return None
  ...
  if runner_type == _SNPE_RUNNER:
    ...
    if replacement is not None and replacement is not active_bundle and bundle_files_ready(replacement):
      ...
      if replacement is not None and getattr(replacement, "index", None) is not None and params.get(_DOWNLOAD_INDEX_KEY) is None:
        ...
        cloudlog.warning(f"Queued tinygrad migration for legacy bundle {getattr(active_bundle, 'internalName', '<unknown>')}")
