...
class AlwaysOnLateral:
  ...
  def _has_runtime_lkas_toggle(self, CS: structs.CarState) -> bool:
    lateral_available = CS.lateralAvailable or CS.cruiseState.available
  ...
  def _cruise_main_available(CS: structs.CarState) -> bool:
    available = bool(getattr(getattr(CS, 'cruiseState', None), 'available', False))
    return available or bool(getattr(CS, 'cruiseFaultLateralMode', False))
